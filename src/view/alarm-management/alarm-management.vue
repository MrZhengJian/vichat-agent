...
<template>
	<div class="alarm">
		<div class="btns">
			<!-- <Input @on-keyup="_searchText" v-model="searchText" icon="md-search" :placeholder="alarm_search_placeholder" style="width: 300px;float:left"></Input> -->
	    ...
	    <Modal v-model="modal2">
	        ...
	        <h1 style="text-align:center;margin:20px 0">{{$t('close_confirm_content')}}</h1>
	        <div slot="footer">
	            <Button type="primary" size="large" @click="closeAlarm" >{{$t('ok')}}</Button>
	        </div>
	    </Modal>
		              ...
		              h('Button',
		                {
		                  ...
		                  style: {
		                    display: 'inline-block',
		                    color: (params.row.alarmState==this.$t('alarmStateList1')?'#2DB7F5':'#ccc'),
		                    cursor: 'pointer'
		                  },
		                  props: {
		                    type: 'text',
		                    size: 'small',
		                    disabled:(params.row.alarmState==this.$t('alarmStateList1')?false:true)
		                  }
		                }, this.$t('finish'))
		            ])
		          }
	...
	methods: {
		_getMes(){
			...
			let params={
				...
				'startTime':this.DatePickervalue[0]==""?"":dateFormat(new Date(this.DatePickervalue[0]),'yyyy-MM-dd hh:mm:ss'),
            	'endTime':this.DatePickervalue[0]==""?"":dateFormat(new Date(this.DatePickervalue[1]),'yyyy-MM-dd hh:mm:ss'),
			}
			queryAlarm(params)
			.then(function(res){
