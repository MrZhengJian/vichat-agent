    ...
    <div class="channel-details">
        ...
        <Modal
            ...
            <div class="tree">
                ...
                <org-tree v-show="showTree" :partyId="partyId" v-on:changeOrg="changeOrg"></org-tree>
        ...
        <Modal :title="channel_details_modal2_title" v-model="modal3">
            ...
            <div slot="footer">
                ...
                <Button type="primary" size="large" @click="deleteUsers('null')">
                    {{$t('ok')}}
                </Button>
            </div>
        </Modal>
...
export default {
  ...
  methods: {
    ...
    confirmDeletion () {
      let _this = this
      this.deleteUsers(this.DeleteItem)
      this.modal2 = false
    },
    ...
    deleteUsers (n) {
      ...
      if (n != 'null') {
        ...
        selected = this.selectionUid
      }
      let _this = this
      for (let i = 0; i < selected.length; i++) {
        switch (selected[i].adminGrade) {
          case this.$t('channel_member5') :
            selected[i].adminGrade = 5
            break
          case this.$t('channel_member10') :
            selected[i].adminGrade = 10
            break
        }
        switch (selected[i].priority) {
          case this.$t('highest') :
            selected[i].priority = 10
            break
          case this.$t('high') :
            selected[i].priority = 3
            break
          case this.$t('middle') :
            selected[i].priority = 2
            break
          case this.$t('low') :
            selected[i].priority = 1
            break
        }
        if (selected[i].videolen == this.$t('Infinite')) {
          selected[i].videolen = 0
        }
      }
      batchDeleteRoomMember(selected)
        .then(function (res) {
          _this.modal3 = false
