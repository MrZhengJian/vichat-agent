...
export default {
      ...
      columns: [
          ...
          render: (h, params) => {
            return h('Tooltip', {
              ...
              params.row.rid,
              h('span', { slot: 'content', style: { whiteSpace: 'normal', wordBreak: 'break-all' } }, params.row.rid)
            ])
          }
  ...
  created () {
    ...
    // loadRoomCache().then((res) => {

    //   this.channelTypeList = res.data.obj[4].Type_StyleId
    //   this.pttCtrlList = res.data.obj[3].Type_PttCtrl

    //   this.turn_type('Type_StyleId',this.channelTypeList)
    //   this.turn_type('Type_PttCtrl',this.pttCtrlList)
  ...
  methods: {
    ...
    // _getChannelList () { // 获取所有频道，用于搜索(在以获取的所有频道中搜索，不需要在请求)
    //   let _this = this
    //   queryChannels()
    //     .then(function (res) {
    //       _this.pages.total = res.data.count
    ...
    _addChannel () {
      ...
      if (this.channelForm.style == '') {
        this.$Message.error(this.$t('channel_addChannel_error2'))
        return
      }
      let _this = this
      // console.log(this.channelForm)
      this.channelForm.partyId = Number(this.partyId)
